...
uses
  System.Classes, System.UITypes, FMX.ListBox, FMX.StdCtrls, FMX.ExtCtrls,
  FMX.Layouts, FMX.Controls,
  FMX.Presentation.Style, FMX.Calendar, FMX.Presentation.Messages,
...
type
  ...
  protected
    { Messages from Model }
    procedure MMDateChanged(var AMessage: TDispatchMessageWithValue<TDateTime>);
      message MM_DATE_CHANGED;
    procedure MMWeekNumbersChanged(var AMessage
      : TDispatchMessageWithValue<Boolean>); message MM_WEEKNUMBERS_CHANGED;
    procedure MMFirstDayOfWeekChanged(var AMessage
      : TDispatchMessageWithValue<TCalDayOfWeek>);
      message MM_FIRSTDAYOFWEEK_CHANGED;
  ...
  protected
    ...
    procedure DoDayClick(const Sender: TCustomListBox;
      const Item: TListBoxItem);
    procedure DoDaysMouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Single);
    procedure DoDaysMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Single);
    { inherited }
...
uses
  System.SysUtils, System.DateUtils, System.Math, FMX.Types, FMX.Platform,
  FMX.Presentation.Factory, FMX.ActnList;
...
constructor TDayItem.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
{$IFDEF ANDROID}
  Self.Font.Size := 14; // Задайте размер шрифта на ваше усмотрение
{$ENDIF}
{$IFDEF MSWINDOWS}
  Self.Font.Size := 13; // Задайте размер шрифта на ваше усмотрение
{$ENDIF}
  Self.StyledSettings := Self.StyledSettings - [TStyledSetting.Size];
  // Отключаем размер шрифта по умолчанию
end;
...
procedure TStyledCalendar.ApplyStyle;
  ...
  function FindButtonAndAssignClickHandler(const AResourceName: string;
...
procedure TStyledCalendar.DoDayClick(const Sender: TCustomListBox;
  const Item: TListBoxItem);
begin
...
procedure TStyledCalendar.DoDaysMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Single);
begin
  if FDisableDayChange = 0 then
...
procedure TStyledCalendar.DoDaysMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Single);
begin
  if FDisableDayChange > 0 then
  begin
...
function GregorianFirstWeekDaysCount(const AYear: Word;
  const AFirstDayOfWeek: Word): Word;
var
...
begin
  ...
  Result := DaysPerWeek - ((FirstJanuaryDay - AFirstDayOfWeek + 7)
    mod DaysPerWeek);
end;

function GregorianWeeksInAYear(const AYear: Word;
  const AFirstDayOfWeek: Word): Word;
var
...
function GregorianWeekOfTheYear(const ADateTime: TDateTime;
  const AFirstDayOfWeek: Word): Word;
var
  DaysInFirstWeek: Word;
begin
  DaysInFirstWeek := GregorianFirstWeekDaysCount(ADateTime.Year,
    AFirstDayOfWeek);
  if DaysInFirstWeek >= 4 then
    // 1 January - 1 week number
    Result := (DayOfTheYear(ADateTime) + DaysPerWeek - DaysInFirstWeek -
      1) div 7 + 1
...
procedure TStyledCalendar.FillWeeksNumbers;
  ...
  begin
    ...
    FirstWeekOfMonth := GregorianWeekOfTheYear(RecodeDay(FDateTime, 1),
      FirstDayOfWeek);
  ...
  var
    ...
    WeekCorrection: Integer;
  begin
    FirstWeekOfJanuary := GregorianWeekOfTheYear(RecodeDay(FDateTime, 1),
      FirstDayOfWeek);
  ...
  begin
    ...
    FirstWeekOfDecember := GregorianWeekOfTheYear(RecodeDay(FDateTime, 1),
      FirstDayOfWeek);
    LastWeekOfDecember := GregorianWeekOfTheYear(RecodeDay(FDateTime, 31),
      FirstDayOfWeek);
...
procedure TStyledCalendar.KeyDown(var Key: Word; var KeyChar: WideChar;
  Shift: TShiftState);
begin
  inherited;
  case Key of
...
procedure TStyledCalendar.MouseWheel(Shift: TShiftState; WheelDelta: Integer;
  var Handled: Boolean);
begin
  inherited;
  if not Handled then
...
function TStyledCalendar.TryFindDayItem(const ADateTime: TDateTime)
  : TListBoxItem;
var
...
begin
  ...
  if DateInRange(ADateTime, FirstVisibleDayItem.Date, LastVisibleDayItem.Date)
  then
...
procedure TStyledCalendar.MMDateChanged(var AMessage
  : TDispatchMessageWithValue<TDateTime>);
begin
  SetDateTime(AMessage.Value);
end;

procedure TStyledCalendar.MMFirstDayOfWeekChanged(var AMessage
  : TDispatchMessageWithValue<TCalDayOfWeek>);
begin
  CalculateFirstDayOfWeek;
  FillCalendar;
...
procedure TStyledCalendar.MMWeekNumbersChanged(var AMessage
...
TPresentationProxyFactory.Current.Register(TCalendar, TControlType.Styled,
  TStyledPresentationProxy<TStyledCalendar>);

finalization

TPresentationProxyFactory.Current.Unregister(TCalendar, TControlType.Styled,
  TStyledPresentationProxy<TStyledCalendar>);
